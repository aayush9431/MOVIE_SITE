...
// Function to fetch thriller movies
async function fetchThrillerMovies(page = 1) {
    try {
        console.log('Fetching thriller movies for page:', page);
        
        // List of thriller movie search terms to get a variety of movies
        const searchTerms = [
            'thriller',
            'suspense',
            'mystery',
...
async function loadMoreMovies() {
    ...
    try {
        ...
        const { movies, totalResults } = await fetchThrillerMovies(currentPage);
        console.log('Fetched movies:', movies);
        
        if (movies.length === 0) {
            if (currentPage === 1) {
                wrapper.innerHTML = '<div class="error-message">No thriller movies found. Please try again later.</div>';
            }
            return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Only run on thriller page
    if (window.location.pathname.includes('Thriller_movie.html')) {
        console.log('On thriller page, loading movies...');
        // Reset page counter when starting fresh
        currentPage = 1;
        loadMoreMovies();
    } else {
        console.log('Not on thriller page:', window.location.pathname);
    }
});