...
async function fetchTopMovies(page = 1) {
    try {
        console.log('Fetching top movies for page:', page);
        
        // List of top movie search terms to get a variety of highly-rated movies
        const searchTerms = [
            'best picture',
            'oscar winner',
            'classic',
...
async function loadMoreMovies() {
    ...
    try {
        ...
        const { movies, totalResults } = await fetchTopMovies(currentPage);
        console.log('Fetched movies:', movies);
        
        if (movies.length === 0) {
            if (currentPage === 1) {
                wrapper.innerHTML = '<div class="error-message">No top movies found. Please try again later.</div>';
            }
            return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Only run on top movies page
    if (window.location.pathname.includes('Top_Movies.html')) {
        console.log('On top movies page, loading movies...');
        // Reset page counter when starting fresh
        currentPage = 1;
        loadMoreMovies();
    } else {
        console.log('Not on top movies page:', window.location.pathname);
    }
}); 